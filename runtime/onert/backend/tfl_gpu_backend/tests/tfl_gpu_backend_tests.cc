...
TEST_F(TflGpuBackendTest, model_with_just_one_AvgPool2D_operation_can_be_evalueated_on_tfl_gpu_bakend) {
...
TEST_F(TflGpuBackendTest, model_with_just_one_Softmax_operation_can_be_evalueated_on_tfl_gpu_bakend) {
...
TEST_F(TflGpuBackendTest, tfl_gpu_backend_should_corretly_work_when_operations_before_and_after_calculated_on_another_backend) {
  ...
  if (platform == "x86_64-linux") {
    ...
    setenv("OP_BACKEND_Conv2D", "acl_neon", 1);
    setenv("OP_BACKEND_DepthwiseConv2D", "tfl_gpu", 1);
  }
  NNFWRuntime nnfw_runtime2;
  nnfw_runtime2.loadGraph(nnfw_ir);
  ...
  ASSERT_TRUE(nnfw_runtime1.numberOfOutputs() == nnfw_runtime2.numberOfOutputs());
  for (size_t i = 0; i < nnfw_runtime1.numberOfOutputs(); ++i) {
    ASSERT_TRUE(isAlmostEqual(nnfw_runtime1.getDataOfOutput(i), nnfw_runtime2.getDataOfOutput(i), 10e-4));
  }
}

TEST_F(TflGpuBackendTest, tfl_gpu_backend_should_corretly_work_when_operations_calculated_on_it_are_before_and_after_operations_calculated_on_another_backend) {
  auto nnfw_ir = makeConv2dDepthwiseConv2dConv2dOperationsSequence();
  ...
  if (platform == "x86_64-linux") {
    setenv("OP_BACKEND_Conv2D", "tfl_gpu", 1);
    setenv("OP_BACKEND_DepthwiseConv2D", "cpu", 1);
  }
  else {
    setenv("OP_BACKEND_Conv2D", "tfl_gpu", 1);
