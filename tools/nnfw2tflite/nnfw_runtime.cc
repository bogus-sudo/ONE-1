...
void NNFWRuntime::loadModelFrom(const std::string& file) {
  operations_graph_ = onert::tflite_loader::loadModel(file.c_str())->at(onert::ir::SubgraphIndex{0});
...
std::shared_ptr<onert::ir::Graph> NNFWRuntime::getGraph() const {
  return operations_graph_;
}
